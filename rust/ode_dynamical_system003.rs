...
impl Config {
    pub fn new() -> Result<Config, &'static str> {
        ...
        let prog_name = "ode-dynamical-system-003";
...
fn main() {
    ...
    measure!({
        for trial in 0..cnf.trial_num {
            ...
            while ((cdata.n as u64) < cnf.nt - 1) && cdata.bound_flg {
                ...
                cdata.x[cdata.n] = cdata.x[cdata.n - 1] + cnf.dt * (-1.0 * cdata.y[cdata.n - 1]);
                cdata.y[cdata.n] = cdata.y[cdata.n - 1] + cnf.dt * (cdata.x[cdata.n - 1]);
    ...
    println!("ode_dynamical_system003_visualize.py で可視化してください. 必要なら適当なところで処理を止めるといいでしょう.");
}
