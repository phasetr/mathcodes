...
impl Config {
    pub fn new() -> Result<Config, &'static str> {
        ...
        let prog_name = "ode-dynamical-system-002";
    ...
    measure!({
        for trial in 0..cnf.trial_num {
            ...
            while ((cdata.n as u64) < cnf.nt - 1) && cdata.bound_flg {
                ...
                cdata.x[cdata.n] = cdata.x[cdata.n - 1]
                    + cnf.dt * (3.0 * cdata.x[cdata.n - 1] + 1.0 * cdata.y[cdata.n - 1]);
                cdata.y[cdata.n] = cdata.y[cdata.n - 1]
                    + cnf.dt * (-5.0 * cdata.x[cdata.n - 1] - 1.0 * cdata.y[cdata.n - 1]);
