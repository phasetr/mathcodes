...
def main(argv = sys.argv):
    ...
    for i, fname in enumerate(file_names):
        ...
        plots = [f""""{fn}" u 1:2 pt 7 ps 0.1 lc rgb '#99ccff'""" for fn in  fnames]
        plot_cmd = ",".join(plots)
        cmd = f"""set terminal png
set datafile separator ","
set output "{png_save_dir}/img.{p.stem}.png"
unset key
set ticslevel 0
...
"""
    ...
    print(datetime.datetime.now())
    gpname = "ode_dynamical_system003_visualize.tmp.gp"
    ...
    run(["ffmpeg", "-framerate", "30", "-i", f"{png_save_dir}/img.%08d.png", f"workspace/{target_dir_name}.mp4"])
    print(datetime.datetime.now())
