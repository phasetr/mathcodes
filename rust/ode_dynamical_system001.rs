...
impl Config {
    pub fn new() -> Result<Config, &'static str> {
        ...
        let prog_name = "ode-dynamical-system-001";
...
impl CalcData {
    pub fn new(cnf: &Config) -> CalcData {
        ...
        let x: Array1<f64> = Array::zeros(cnf.total_step as usize);
        let y: Array1<f64> = Array::zeros(cnf.total_step as usize);
        CalcData {
            n: n,
            t: t,
            bound_flg: bound_flg,
    ...
    pub fn write_csv(cnf: &Config, cdata: &CalcData) -> Result<(), Box<dyn error::Error>> {
        ...
        write!(w, "x,y\n").unwrap();
        for i in 0..(cdata.tmp_total_num + 1) {
            let s = format!("{},{}\n", &cdata.x[i], &cdata.y[i]);
...
fn main() {
    ...
    measure!({
        for trial in 0..cnf.trial_num {
            ...
            println!("(x0, y0) = ({}, {})", x0, y0);
            cdata.x[cdata.tmp_total_num] = x0;
            cdata.y[cdata.tmp_total_num] = y0;
            ...
            while ((cdata.n as u64) < cnf.nt) && cdata.bound_flg {
                // カウントアップ
                cdata.n += 1;
                cdata.t = cdata.t + cnf.dt;
                cdata.tmp_total_num += 1;

                cdata.x[cdata.tmp_total_num] = cdata.x[cdata.tmp_total_num - 1]
                    + cnf.dt
                        * (3.0 * cdata.x[cdata.tmp_total_num - 1]
                            + 1.0 * cdata.y[cdata.tmp_total_num - 1]);
                cdata.y[cdata.tmp_total_num] = cdata.y[cdata.tmp_total_num - 1]
                    + cnf.dt
                        * (-5.0 * cdata.x[cdata.tmp_total_num - 1]
                            - 1.0 * cdata.y[cdata.tmp_total_num - 1]);
    ...
    println!("ode_dynamical_system001_visualize.py で可視化してください.");
}

fn bound_check(cnf: &Config, cdata: &CalcData) -> bool {
    cdata.x[cdata.tmp_total_num] < cnf.x_min
        || cnf.x_max < cdata.x[cdata.tmp_total_num]
        || cdata.y[cdata.tmp_total_num] < cnf.y_min
        || cnf.y_max < cdata.y[cdata.tmp_total_num]
}
