// cargo-deps: serde, serde_derive, serde_yaml, ndarray, chrono
extern crate chrono;
...
struct CalcData {
    ...
    gphi: Array4<f64>,     // index function計算用中間変数
    pcap: Array5<f64>,     // index function計算用中間変数
    div_pcap: Array5<f64>, // index function計算用中間変数
...
impl CalcData {
    fn new(cnf: &Config) -> CalcData {
        ...
        let mut cr: Array2<f64> = Array2::zeros((3, 15));
        // 値の代入
        for l in 0..15 {
            cr[(0, l)] = ci[l] as f64;
            cr[(1, l)] = cj[l] as f64;
        ...
        //====================================================================
        //======================phi 計算用変数=================================
        let feq: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        ...
        let grad_phi: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let p0: Array3<f64> = Array::zeros((cnf.imax, cnf.jmax, cnf.kmax));
        let gphi: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let pcap: Array5<f64> = Array::zeros((3, 3, cnf.imax, cnf.jmax, cnf.kmax));
        let div_pcap: Array5<f64> = Array::zeros((3, 3, cnf.imax, cnf.jmax, cnf.kmax));
        let phi_min: f64 = 0.0;
        let phi_max: f64 = 0.0;

        //====================================================================
        //======================圧力・流速計算用変数==========================
        let geq: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        ...
        let delta_p: Array4<f64> = Array::zeros((15, cnf.imax, cnf.jmax, cnf.kmax));
        ...
        let grad_mu: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let grad_u: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let vcap: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        ...
        let grad_rho: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let normal: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));
        let chi: Array3<f64> = Array::zeros((cnf.imax, cnf.jmax, cnf.kmax));
        let fsv: Array4<f64> = Array::zeros((3, cnf.imax, cnf.jmax, cnf.kmax));

        let sigma_temp: f64 = 0.0;

        let krone: Array2<f64> = Array2::eye(3);
                ...
                for i in 0..cnf.imax {
                    ...
                    if is_in_bulk(i, j, k, &cnf) {
                        ...
                        for l in 2..15 {
                            cdata.lap_phi[(i, j, k)] = cdata.lap_phi[(i, j, k)]
                                + cdata.phi[(
                                    perx(((i as i64) + cdata.ci[(l)]) as usize, &cnf),
                                    pery(((j as i64) + cdata.cj[(l)]) as usize, &cnf),
                                    perz(((k as i64) + cdata.ck[(l)]) as usize, &cnf),
                                )];
                        }
                    }
...
fn perx(i: usize, cnf: &Config) -> usize {
    if i > cnf.imax {
        i - cnf.imax
    } else {
        i
...
fn pery(j: usize, cnf: &Config) -> usize {
    if j > cnf.jmax {
        j - cnf.jmax
    } else {
        j
...
fn perz(k: usize, cnf: &Config) -> usize {
    if k > cnf.kmax {
        k - cnf.kmax
    } else {
        k
