...
impl CalcData {
    pub fn new(cnf: &Config) -> CalcData {
        ...
        // initialize
        for i in 0..cnf.nz {
            u[(i, cnf.j_center, cnf.k_center)] =
                f64::exp(-(x[(i)] - x[(cnf.i_center)]).powf(2.0) / 0.001);
        }
        CalcData {
            n: n,
            t: t,
    ...
    pub fn write_csv(cnf: &Config, cdata: &CalcData) -> Result<(), Box<dyn error::Error>> {
        ...
        for i in 0..cnf.nx {
            let s = format!(
                "{},{}\n",
                &cdata.x[i],
                &cdata.u[(i, cnf.j_center, cnf.k_center)],
            );
            write!(w, "{}", s).unwrap();
        }
        w.flush().unwrap();
...
fn main() {
    ...
    while cdata.n <= cnf.nt {
        ...
        for i in 1..(cnf.nx - 2) {
            unew[(i, cnf.j_center, cnf.k_center)] = cdata.u[(i, cnf.j_center, cnf.k_center)]
                + alpha_x
                    * (cdata.u[(i + 1, cnf.j_center, cnf.k_center)]
                        - 2.0 * cdata.u[(i, cnf.j_center, cnf.k_center)]
                        + cdata.u[(i - 1, cnf.j_center, cnf.k_center)]);
        }
