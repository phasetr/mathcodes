...
impl Config {
    pub fn new(args: &[String]) -> Result<Config, &'static str> {
        ...
        let init: Init = match init_yaml.as_str() {
            "sin" => Init::Sin,
            "square" => Init::Square,
        ...
        let reconstruction: Reconstruction = match reconstruction_yaml.as_str() {
        ...
        let limiter_string: String = match limiter {
            ...
            Limiter::FluxLimiter => "FluxLimiter",
        }
        .to_string();

        // riemann の置換処理
        let riemann_yaml: String = cnf_yaml["riemann"].as_str().unwrap().to_string();
        let riemann: Riemann = match riemann_yaml.as_str() {
            "roe" => Riemann::Roe,
        ...
        let riemann_string: String = match riemann {
            Riemann::Roe => "Roe",
            Riemann::FluxLimiter => "FluxLimiter",
        }
        .to_string();
