...
fn main() {
    ...
    let dir_name = Local::now()
        .format(&format!(
            "workspace/%Y%m%d-%H%M%S_4-6_convection_2nd_order_TVD_{}",
            &choice_string,
        ))
        .to_string();
    ...
    while t <= tstop {
        ...
        for i in 1..(NODE - 2) {
            ...
            // セル境界 (i+1/2) 左側の値
            uls[i + 1] = us[i] + (1.0 - alpha_12 * dt / dx) * phi_l * delta_l;
            // セル境界 (i+1/2) 右側の値
            urs[i + 1] = us[i + 1] - (1.0 + alpha_12 * dt / dx) * phi_r * delta_r;
        }
