...
impl CalcData {
    pub fn new(cnf: &Config) -> CalcData {
        ...
        let u_pml = Array::zeros(cnf.nx);
        let w_pml = Array::zeros(cnf.nx);
        let u_ord = Array::zeros(cnf.nx);
...
fn main() {
    ...
    let alpha_test = cnf.dt / cnf.dx.powf(2.0);
    ...
    while cdata.n <= cnf.nt {
        ...
        cdata.u_pml[cnf.nx / 2 - 1] = cnf.a * f64::sin(2.0 * PI * cnf.f * cdata.t);

        // u_pml と w_pml の計算はわける必要あり.
        for i in 1..(cnf.nx - 1) {
            u_pmlnew[i] = cdata.u_pml[i] + alpha * (cdata.w_pml[i] - cdata.w_pml[i - 1])
                - cdata.sigma[i] * cdata.u_pml[i];
        }
        for i in 0..(cnf.nx - 2) {
            // for の開始位置に注意
            w_pmlnew[i] = cdata.w_pml[i] + alpha * (u_pmlnew[i + 1] - u_pmlnew[i])
        ...
        cdata.u_ord[cnf.nx / 2 - 1] = cnf.a * f64::sin(2.0 * PI * cnf.f * cdata.t);

        for i in 1..(cnf.nx - 2) {
            ut_ordnew[i] = cdata.ut_ord[i]
                + alpha_test * (cdata.u_ord[i - 1] - 2.0 * cdata.u_ord[i] + cdata.u_ord[i + 1]);
            u_ordnew[i] = cdata.u_ord[i] + cnf.dt * ut_ordnew[i];
        }

        // bc: ディリクレ境界条件
        u_ordnew[0] = 0.0;
        u_ordnew[cnf.nx - 1] = 0.0;
