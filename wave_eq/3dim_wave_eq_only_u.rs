...
impl Config {
    pub fn new() -> Result<Config, &'static str> {
        ...
        let prog_name = "3dim-wave-eq-only-y";
        let title = Local::now()
...
struct CalcData {
    ...
    pub uold: Array3<f64>, // u の 1 ステップ前の時間
}
...
impl CalcData {
    pub fn new(cnf: &Config) -> CalcData {
        ...
        let uold = Array::zeros((cnf.nx, cnf.ny, cnf.nz));
        CalcData {
            n: n,
            t: t,
...
fn main() {
    ...
    let alpha_z = (cnf.c * cnf.dt / cnf.dz).powf(2.0);
    ...
    while cdata.n <= cnf.nt {
        ...
        for i in 1..(cnf.nx - 2) {
            for j in 1..(cnf.ny - 2) {
                for k in 1..(cnf.nz - 2) {
                    unew[(i, j, k)] = 2.0 * cdata.u[(i, j, k)] - cdata.uold[(i, j, k)]
                        + alpha_x
